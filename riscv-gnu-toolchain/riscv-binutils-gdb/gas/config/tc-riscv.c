...
#define USE_BITS(mask,shift)	(used_bits |= ((insn_t)(mask) << (shift)))
  while (*p)
	...
	break;
      ...
      case ']': break;
      case '0': break;
      case 'B': USE_BITS (OP_MASK_RS1,		OP_SH_RS1);	break; // first source register in vector instruction
      case 'H': USE_BITS (OP_MASK_RS2,		OP_SH_RS2);	break; // second source register in vector instruction
      case 'G': USE_BITS (OP_MASK_RS3,		OP_SH_RS3);	break; // third source register in vector instruction
      case 'N': USE_BITS (OP_MASK_RD,		OP_SH_RD);	break; // Destination register in vector instruction
      case 'M': USE_BITS (OP_MASK_M,		OP_SH_M);	break; // vector instruction mask
      case 'O': used_bits |= ENCODE_VITYPE_IMM (-1U); break;
      case 'J': used_bits |= ENCODE_VSTYPE_IMM (-1U); break;
      default:
...
riscv_apply_const_reloc (bfd_reloc_code_real_type reloc_type, bfd_vma value)
{
  switch (reloc_type)
    ...
    case BFD_RELOC_RISCV_LO5_VS:
      return ENCODE_VSTYPE_IMM (value);

    case BFD_RELOC_RISCV_LO5_VI:
      return ENCODE_VITYPE_IMM (value);
...
static void
pcrel_vstore (int srcreg, int tempreg, expressionS *ep, const char *lo_insn,
	     bfd_reloc_code_real_type hi_reloc,
	     bfd_reloc_code_real_type lo_reloc)
{
  pcrel_access (srcreg, tempreg, ep, lo_insn, "G,s,J", hi_reloc, lo_reloc);
}

static void
pcrel_vload (int srcreg, int tempreg, expressionS *ep, const char *lo_insn,
	     bfd_reloc_code_real_type hi_reloc,
	     bfd_reloc_code_real_type lo_reloc)
{
  pcrel_access (srcreg, tempreg, ep, lo_insn, "N,s,O", hi_reloc, lo_reloc);
}
       ...
       bfd_reloc_code_real_type *imm_reloc)
{
  ...
  switch (mask)
    ...
    case M_VLD:
      pcrel_vload (rd, rs1, imm_expr, "vld",
         	  BFD_RELOC_RISCV_PCREL_HI20, BFD_RELOC_RISCV_PCREL_LO5_VI);
      break;
    ...
    case M_FSD:
     ...
     case M_VSD:
      pcrel_vstore (rs3, rs1, imm_expr, "vsd",
         	   BFD_RELOC_RISCV_PCREL_HI20, BFD_RELOC_RISCV_PCREL_LO5_VS);
      break;
...
static const struct percent_op_match percent_op_vitype[] =
{
  {"%lo", BFD_RELOC_RISCV_LO5_VI},
  {"%tprel_lo", BFD_RELOC_RISCV_TPREL_LO5_VI},
  {"%pcrel_lo", BFD_RELOC_RISCV_PCREL_LO5_VI},
  {0, 0}
};

static const struct percent_op_match percent_op_vstype[] =
{
  {"%lo", BFD_RELOC_RISCV_LO5_VS},
  {"%tprel_lo", BFD_RELOC_RISCV_TPREL_LO5_VS},
  {"%pcrel_lo", BFD_RELOC_RISCV_PCREL_LO5_VS},
  {0, 0}
};
...
rvc_lui:
	      ...
	      if (reg_lookup (&s, RCLASS_FPR, &regno))
		{
	      ...
	      break;
       case 'M':		/* vector masking mode.  */
          if (arg_lookup (&s, riscv_vector_mode, ARRAY_SIZE (riscv_vector_mode), &regno))
          {
            INSERT_OPERAND (M, *ip, regno);
            continue;
          }
          break;
        case 'N':		/* Vector rd.  */
        ...
        case 'J': /* Store displacement.  */
          p = percent_op_vstype;
          *imm_reloc = BFD_RELOC_RISCV_LO5_VS;
          goto load_store;
        case 'O': /* Load displacement.  */
          p = percent_op_vitype;
          *imm_reloc = BFD_RELOC_RISCV_LO5_VI;
          goto load_store;
...
md_apply_fix (fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  switch (fixP->fx_r_type)
    ...
    case BFD_RELOC_RISCV_TPREL_LO5_VI:
    case BFD_RELOC_RISCV_TPREL_LO5_VS:
    ...
    case BFD_RELOC_RISCV_PCREL_LO5_VS:
    case BFD_RELOC_RISCV_PCREL_LO5_VI:
