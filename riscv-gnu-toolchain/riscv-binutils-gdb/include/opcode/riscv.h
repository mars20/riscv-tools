...
static const char * const riscv_vector_mode[4] =
{
  "s", "v", "f", "t"
};
...
#define EXTRACT_VITYPE_IMM(x) \
  (RV_X(x, 27, 5) | (RV_IMM_SIGN(x) << 5 ))
#define EXTRACT_VSTYPE_IMM(x) \
  (RV_X(x, 7, 5) | (RV_IMM_SIGN(x) << 5 ))
...
#define ENCODE_VITYPE_IMM(x) \
  (RV_X(x, 0, 5) << 27)
#define ENCODE_VSTYPE_IMM(x) \
  (RV_X(x, 0, 5) << 7)
...
#define VALID_VITYPE_IMM(x) (EXTRACT_VITYPE_IMM(ENCODE_VITYPE_IMM(x)) == (x))
#define VALID_VSTYPE_IMM(x) (EXTRACT_VSTYPE_IMM(ENCODE_VSTYPE_IMM(x)) == (x))
...
#define OP_SH_RL		25
#define OP_MASK_M               0x3  //vector mask
#define OP_SH_M                 12
...
struct riscv_opcode
{
  ...
  const char *name;
  /* The ISA subset name (I, M, A, F, D, V, Xextension).  */
  const char *subset;
