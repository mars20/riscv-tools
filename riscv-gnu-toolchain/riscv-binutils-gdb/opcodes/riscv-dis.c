  ...
  for (; *d != '\0'; d++)
	...
	case 'R':
	  ...
	  break;
          ...
          case 'M':
           arg_print (info, EXTRACT_OPERAND (M, l),
		     riscv_vector_mode, ARRAY_SIZE (riscv_vector_mode));
           break;
          case 'N':
          ...
          case 'B':
            ...
            break;
          case 'H':
            print (info->stream, "%s", riscv_vpr_names[EXTRACT_OPERAND (RS2, l)]);
            break;
          case 'G':
             print (info->stream, "%s", riscv_vpr_names[EXTRACT_OPERAND (RS3, l)]);
            break;
          case 'J':
            maybe_print_address (pd, rs1, EXTRACT_VSTYPE_IMM (l));
	    print (info->stream, "%d", (int)EXTRACT_VSTYPE_IMM (l));
            break;
          case 'O':
            maybe_print_address (pd, rs1, EXTRACT_VITYPE_IMM (l));
            print (info->stream, "%d", (int)EXTRACT_VITYPE_IMM (l));
            break;
	case 'E':
	  {
