...
void processor_t::parse_isa_string(const char* str)
{
  ...
  if (supports_extension('V') && !supports_extension('F'))
    bad_isa_string(str);

  if (supports_extension('V') && max_xlen < 64)
    bad_isa_string(str);
...
static unsigned mvlen(unsigned vlen, unsigned maxew)
{
  switch(maxew)
  {
    case 1:
      return (vlen >> OFFSETMAXEW8);
    case 2:
      return (vlen >> OFFSETMAXEW16);
    case 3:
      return (vlen >> OFFSETMAXEW32);
    case 4:
      return (vlen >> OFFSETMAXEW64);
  }
  abort();
}
...
void processor_t::set_csr(int which, reg_t val)
{
  ...
  switch (which)
  {
    ...
    case CSR_FCSR:
      ...
      break;
    case CSR_VEMAXW:
      state.vemaxw = val & (VEMAXW >> VEMAXW_SHIFT);
      break;
    case CSR_VREGMAX:
      state.vregmax = val & (VREGMAX >> VREGMAX_SHIFT);
      break;
    case CSR_VCFG:
      state.vemaxw = (val & VEMAXW) >> VEMAXW_SHIFT;
      state.vregmax = (val & VREGMAX) >> VREGMAX_SHIFT;
      break;
    case CSR_VL:
        state.vl = val <= mvlen(max_vlen, state.vemaxw)? val: mvlen(max_vlen, state.vemaxw);
      break;
    case CSR_MSTATUS: {
...
reg_t processor_t::get_csr(int which)
{
  ...
  switch (which)
  {
    ...
    case CSR_VREGMAX:
      if(!supports_extension('V'))
        break;
      return state.vregmax;
    case CSR_VCFG:
      if(!supports_extension('V'))
        break;
      return (state.vemaxw << VEMAXW_SHIFT) | (state.vregmax << VREGMAX_SHIFT);
    case CSR_VL:
      if(!supports_extension('V'))
        break;
     return state.vl;
