...
#define VEMAXW_SHIFT 8
#define VEMAXW (0x7 << VEMAXW_SHIFT)
#define VREGMAX_SHIFT 0
#define VREGMAX (0xFF << VREGMAX_SHIFT)

enum mem_byte_offset
{
  MemByteOffset8 = 0,
  MemByteOffset16 = 1,
  MemByteOffset32 = 2,
...
public:
  ...
  /* Vector instruction decoding fuctions */
  uint64_t m() { return x(12, 2); } // vector mask
  uint64_t vi_imm() { return int64_t(b) >> 27;}
  uint64_t vs_imm() { return xs(7,5);}
...
# define WRITE_VREG(reg, value) DO_WRITE_VREG(reg, value)
#else
...
# define WRITE_VREG(reg, value) ({ \
    vreg_t wdata = vreg(value); /* value may have side effects */ \
    STATE.log_reg_write = (commit_log_reg_t){((reg) << 1) | 1, {wdata.e64[3], wdata.e64[0]}}; \ //TODO: Currently printing as fp
    DO_WRITE_VREG(reg, wdata); \
  })
#endif

// Vector register type
typedef vec256_t vreg_t;
// Vector Extension Macros
...
#define VMASK READ_VREG(1)
#define DO_WRITE_VREG(reg, value) (STATE.VPR.write(reg, value))
#define WRITE_VRD(value) WRITE_VREG(insn.rd(), value)
#define EW ({ reg_t ew = STATE.vemaxw;\
             ew;})
#define VL ({ reg_t vl = STATE.vl; \
             vl;})

inline vreg_t vreg(uint256_t v) {
  static vec256_t temp;
  temp.e32[0] = v.ele[0];
  temp.e32[1] = v.ele[1];
  temp.e32[2] = v.ele[2];
  temp.e32[3] = v.ele[3];
