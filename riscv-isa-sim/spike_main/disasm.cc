...
struct : public arg_t {
  std::string to_string(insn_t insn) const {
    return std::to_string((int)insn.m());
  }
} vmask;

struct : public arg_t {
  std::string to_string(insn_t insn) const {
    return std::to_string((int)insn.vi_imm()) + '(' + xpr_name[insn.rs1()] + ')';
  }
} vload_address;

struct : public arg_t {
  std::string to_string(insn_t insn) const {
    return std::to_string((int)insn.vs_imm()) + '(' + xpr_name[insn.rs1()] + ')';
  }
} vstore_address;
...
disassembler_t::disassembler_t(int xlen)
{
  ...
  #define DEFINE_VITYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &vload_address, &xrs2, &vmask})
  #define DEFINE_VSTYPE(code) DISASM_INSN(#code, code, 0, {&vrs3, &vstore_address, &xrs2, &vmask})
  #define DEFINE_VIXTYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &vload_address, &vrs2, &vmask})
  #define DEFINE_VSXTYPE(code) DISASM_INSN(#code, code, 0, {&vrs3, &vstore_address, &vrs2, &vmask})
  #define DEFINE_VRTYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &vrs1, &vrs2, &vmask})
  #define DEFINE_VR2TYPE(code) DISASM_INSN(#code, code, 0, {&vrd, &vrs1, &vrs2, &vrs3, &vmask})
  ...
  DEFINE_VRTYPE(vadd);
  DEFINE_VRTYPE(vmul);
  DEFINE_VR2TYPE(vmadd);
  DEFINE_VRTYPE(vfadd);
  DEFINE_VRTYPE(vfmul);
  DEFINE_VR2TYPE(vfmadd);
  DISASM_INSN("vredmax", vredmax, 0, {&vrd, &vrs1})
  DISASM_INSN("vredmin", vredmin, 0, {&vrd, &vrs1})
  DISASM_INSN("vredsum", vredsum, 0, {&vrd, &vrs1})
  DISASM_INSN("vfredsum", vfredsum, 0, {&vrd, &vrs1})
  DEFINE_VRTYPE(vmax);
  DEFINE_VRTYPE(vmin);
  DEFINE_VRTYPE(vfmax);
  DEFINE_VRTYPE(vfmin);
  DISASM_INSN("vsplat", vsplat, 0, {&vrd, &vrs1, &xrs2, &vmask})
  DISASM_INSN("vbcastf", vbcastf, 0, {&vrd, &frs1})
  DISASM_INSN("vbcastx", vbcastx, 0, {&vrd, &xrs1})
  DEFINE_VRTYPE(vmerge);
  DISASM_INSN("vinsx", vinsx, 0, {&vrd, &xrs1, &xrs2, &vmask})
